...
void bxCAN::transmit(uint16_t id, uint8_t length, uint8_t data[]){
    ...
    CANx->sTxMailBox[mailbox].TIR = ((id << CAN_TI0R_STID_Pos) & 0xFFE00000);
    ...
    for(int i = 0; i < 32; i++){
        usart1_printf("%d ", CANx->sTxMailBox[mailbox].TIR >> (31-i) & 0x01);
    }
    usart1_printf("\n");
    for(int i = 0; i < 32; i++){
        usart1_printf("%d ", CANx->sTxMailBox[mailbox].TDTR >> (31-i) & 0x01);
    }
    usart1_printf("\n");
    for(int i = 0; i < 32; i++){
        usart1_printf("%d ", CANx->sTxMailBox[mailbox].TDHR >> (31-i) & 0x01);
    }
    usart1_printf("\n");
    for(int i = 0; i < 32; i++){
        usart1_printf("%d ", CANx->sTxMailBox[mailbox].TDLR >> (31-i) & 0x01);
    }
    usart1_printf("\n");

    //request transmission
    CANx->sTxMailBox[mailbox].TIR |= CAN_TI0R_TXRQ;
    //while(CANx->sTxMailBox[mailbox].TIR & CAN_TI0R_TXRQ){
    //    usart1_printf("%d", CANx->sTxMailBox[mailbox].TIR & CAN_TI0R_TXRQ);
