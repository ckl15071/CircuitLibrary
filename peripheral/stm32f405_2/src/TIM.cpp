...
void TIM::setupPwmOutput(TIM_TypeDef *tim,
    ...
    //よろしくない場合分け
    //TODO: ペリフェラルの周波数から出力周波数計算
    if(TIMx == TIM12) TIMx->PSC = 24-1; //50Hz
    if(TIMx == TIM1) TIMx->PSC = 12-1;
...
//pwm 2ch for motor
void TIM::setupPwmOutputMotor12(TIM_TypeDef *tim,
        GPIO_TypeDef *pwm_ch1, uint8_t ch1_pin,
        GPIO_TypeDef *pwm_ch2, uint8_t ch2_pin, uint16_t period){
    ...
    //よろしくない場合分け
    //TODO: ペリフェラルの周波数から出力周波数計算
    if(TIMx == TIM1) TIMx->PSC = 24-1;
    if(TIMx == TIM5) TIMx->PSC = 24-1;

    //TIMx->PSC = 48-1;
    TIMx->ARR = 60000-1;    //100Hz
    TIMx->CCR1 = 3000-1;    //1ms
    TIMx->CCR2 = 3000-1;    //1ms
...
//pwm 2ch for motor
//ch3 ch4
void TIM::setupPwmOutputMotor34(TIM_TypeDef *tim,
        GPIO_TypeDef *pwm_ch1, uint8_t ch1_pin,
        GPIO_TypeDef *pwm_ch2, uint8_t ch2_pin, uint16_t period){

    this->TIMx = tim;
    this->period = period;
    ch3 = new Channel(tim, 3, pwm_ch1, ch1_pin, period);
    ch4 = new Channel(tim, 4, pwm_ch2, ch2_pin, period);

    clockEnable();
    setAlternate();

    TIMx->CR1 |= TIM_CR1_ARPE;
    TIMx->CCMR2 |= 0x6868;  //ch3,ch4 PWM1 mode

    //これ使うにはchを選択できるようにしないといけない
    TIMx->CCER |= (TIM_CCER_CC3E | TIM_CCER_CC4E);  //ピン出力

    //よろしくない場合分け
    //TODO: ペリフェラルの周波数から出力周波数計算
    if(TIMx == TIM1) TIMx->PSC = 12-1;
    if(TIMx == TIM5) TIMx->PSC = 12-1;
    //TIMx->PSC = 48-1;

    TIMx->ARR = 60000-1;    //100Hz
    TIMx->CCR3 = 3000-1;    //1ms
    TIMx->CCR4 = 3000-1;    //1ms
